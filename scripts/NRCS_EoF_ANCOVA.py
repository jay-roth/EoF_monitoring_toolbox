...
def transformData(df, par_col, sit_col, phs_col, alpha=0.025, min_pass=2):
    ...
    for s in df[sit_col].unique():
        for p in df[phs_col].unique():
            raw_dat.append(df[(df[sit_col]==s) &\
                              (df[phs_col]==p)][par_col].values)
...
if __name__ == "__main__":
    ...
    param_file = 'NRCS_EoF_ANCOVA_input.txt'
                    ...
                    for ts in tr_sites:
                        ...
                        for obs_col in obs:
                            ...
                            if err == 0:
                                ...
                                ct_data.sort_values(params['date']['val'])
                                ...
                                tr_data = pdf[(pdf[type_col]!=types[0]) &\
                                               (df[obs_col]>0)][[date_col, 
                                                                 phase_col, 
                                                                 obs_col]]
                                tr_data.sort_values(params['date']['val'])
